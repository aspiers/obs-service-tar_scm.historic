...
class GitTests(TestsBase):
    ...
    repo_dir     = fixtures_dir + '/repo.git'
    repo_url     = 'file://' + repo_dir

    tar_scm_bin  = tests_dir + '/../tar_scm'
    if not os.access(tar_scm_bin, os.X_OK):
        raise RuntimeError, "Failed to find tar_scm at " + tar_scm_bin

    @classmethod
    def setup_fixtures(cls):
        if os.path.exists(cls.fixtures_dir):
            shutil.rmtree(cls.fixtures_dir)
        os.mkdir(cls.fixtures_dir)
